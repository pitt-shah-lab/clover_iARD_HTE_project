...
# Covariate-balanced allocation procedure from SIDES package

# Xcov is a dataframe containing just the X variables in the dataset to be split
# type_var is a string of length nrow(Xcov) specifying the type of covariate. Recognizes "continuous" vs something else (e.g. "nominal")
# H is the number of datasets you want after splitting (default to 2)
# pct_random is the number of patients to send to each dataset completely at random (initialize the two)
# prop_gpe is a vector of proportions that sum to 1, indicating proportions of total sample in each dataset after split

#library(nnet)

allocation_procedure = function(Xcov,
                                type_var,
                                H=2, 
                                pct_random=0.5, 
                                prop_gpe=c(0.5,0.5), 
  ...
  if(H > 1){
    ...
    # Categorize the continuous variables into 3 buckets
    if(nb_cont>0){
      for(j in 1:nb_cont){
        #browser()
        var<-Xcov[,ind_cont[j]]
        quant = quantile(Xcov[,ind_cont[j]], probs = seq(0, 1, 0.33),na.rm=TRUE)
        q33 = as.numeric(quant[2])
        q66 = as.numeric(quant[3])
        for(i in 1:nb_patients){
          if(is.na(var[i])){
            X[i,ind_cont[j]] = -1 #NAs get assigned -1
          }
          else if(var[i] < q33){
            X[i,ind_cont[j]] = 0
          }
